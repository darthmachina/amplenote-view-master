const plugin = 
{
  ...
  _getInlineFields(settings, noteContent) {
    ...
    while(match = this.constants.inlineFieldRegexp.exec(noteContent)) {
      if (!settings.includeFields || settings.includeFields.includes(match[1])) {
        noteFields.set(match[1],match[2]);
      }
    }

    var fields = [];
    if (!settings.includeFields) {
      noteFields.keys().forEach(noteField => {
        fields.push(new this.InlineField(noteField, noteFields.get(noteField)));
      });
    } else {
      settings.includeFields.forEach(includeField => {
        if (noteFields.has(includeField)) {
          fields.push(includeField, noteFields.get(includeField));
        }
      });
    }
    while(match = this.constants.inlineFieldRegexp.exec(noteContent)) {
  ...
  _getCellContentForGallery(settings, note) {
  ...
  _getGroupContentForGallery(settings, groupTag, notes) {
    ...
    const notesString = notes
      .map(note => {
        return this._getCellContentForGallery(settings, note);
      })
  ...
  _getRowContentForTable(settings, note) {
    return `|${(note.image ? `![${note.image.image}\\|${settings.imageWidth}](${note.image.url})` : "")}|[${note.title}](https://www.amplenote.com/notes/${note.uuid})|${settings.includeFields.map(field => this._findFieldValue(note, field)).join("|")}|`;
  },
  ...
  _getGroupContentForTable(settings, groupTag, notes) {
    ...
    const initialValue = `||**Note**|${settings.includeFields.join("|")}|\n|-|-${"|-".repeat(settings.includeFields.length)}|\n`;

    const notesString = notes
      .map(note => {
        return this._getRowContentForTable(settings, note);
      })
      .reduce(reducer, initialValue);
