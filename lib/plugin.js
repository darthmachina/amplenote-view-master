const plugin = 
{
  ...
  Settings: class {
    constructor(queryTag, groupTags, imageWidth, tableColumns, includeFields, sortBy, displayAs, progressCompletion, progressRemaining, showImage) {
    ...
    copy(
      ...
      return new Settings(queryTag, groupTags, imageWidth, tableColumns, includeFields, sortBy, displayAs, progressCompletion, progressRemaining, showImage);
    }
  },
  ...
  _createSettingsTable(settings, defaultImageWidth, defaultTableColumns) {
    ...
    return `| | |\n|-|-|\n|Query Tag|${settings.queryTag}|\n|Group By Tags|${settings.groupTags}|\n${imageCell}${columnsCell}${includeFieldsCell}${sortByCell}${displayAsCell}${showImageCell}`
  },
  ...
  _parseToday(today) {
    ...
    const monthNumber = this.constants.monthNameToNumber[todaySplit[0]];
    const dayOfMonth = todaySplit[1].replace(/(st,|rd,|th,|nd,)/, "");
    return Date.parse(`${todaySplit[2]}-${monthNumber}-${dayOfMonth}}`);
  },
    ...
    if (settings.sortBy) {
      return notes.toSorted((current, next) => {
        ...
        if (Number.isNaN(currentSortBy) || Number.isNaN(nextSortBy)) {
          ...
          const currentDateValue = this._parseToday(currentSortValue);
          const nextDateValue = this._parseToday(nextSortValue);
          ...
          if (Number.isNaN(currentDateValue) || Number.isNaN(nextDateValue)) {
            ...
            return currentSortValue > nextSortValue ? 1 : -1;
          }
        }
