const plugin = 
{
  ...
  async _getNotesForGroup(app, settings, queryTag, groupTag) {
    const filteredNotes = await app.filterNotes({ tag: `${queryTag},${groupTag}` });
    const fullNotes = await filteredNotes.map(async noteHandle => {
      ...
      const myNote = new this.Note(noteHandle.uuid, noteHandle.name, noteContent, inlineFields, image, tasks.length, this._completedTaskCount(tasks));
      return myNote;
    });

    const notes = await Promise.all(fullNotes);
