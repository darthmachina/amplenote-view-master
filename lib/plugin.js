const plugin = 
{
  ...
  _getRowContentForTable(settings, note) {
    return `|${(note.image ? `![${note.image.image}\\|${settings.imageWidth}](${note.image.url})` : "")}|[${note.title}](https://www.amplenote.com/notes/${note.uuid})|${settings.includeFields.map(field => note.inlineFields.get(field)?.value ?? "").join("|")}|`;
  },
