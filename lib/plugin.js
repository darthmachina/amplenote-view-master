const plugin = 
{
  ...
  Settings: class {
    constructor(queryTag, groupTags, imageWidth, tableColumns, includeFields, sortBy) {
  ...
  _findFieldValue(note, name) {
    console.log(`_findFieldValue: ${name}`);
    let value = undefined;
    note.inlineFields.forEach(field => {
      if (field.name === name) {
        console.log(`${name} found with value ${field.value}`);
        value = field.value;
      }
    });
    return value;
  ...
  _sortNotes(settings, notes) {
    if (settings.sortBy) {
      console.log(`sortBy set, sorting notes by ${settings.sortBy}`);
      const sorted = notes.toSorted((current, next) => {
        ...
        console.log(`current: ${currentSortBy}, next: ${nextSortBy}`);
        return currentSortBy - nextSortBy;
      });
      sorted.forEach(note => console.log(`note: ${note.title}`));
      return sorted;
    } else {
      return notes;
    }
