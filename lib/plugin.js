const plugin = 
{
  ...
  constants: {
    ...
    includeFieldsRegexp: RegExp("\\|Include Fields\\|([a-z0-9\\-\\/,\\s]+)", "i"),
    sortByRegexp: RegExp("\\|Sort By\\|([a-z0-9\\-\\/,\\s]+)", "i"),
  ...
  _sortNotes(settings, notes) {
    if (settings.sortBy) {
      return notes.sort((current, next) => parseInt(this._findFieldValue(current, settings.sortBy) || "99") - parseInt(this._findFieldValue(next, settings.sortBy) || "99"));
    } else {
      return notes;
    }
},
