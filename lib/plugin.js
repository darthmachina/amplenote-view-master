const plugin = 
{
      ...
      await app
        ...
        .then(handles => {
          return handles.map(async noteHandle => {
            ...
            let tasks = []
            if (settings.includeFields.find(key => key.toLowerCase() === "+progress") != undefined) {
  ...
  _rowFields(settings, note) {
    return `${settings.includeFields.map(field => {
      if (field.startsWith("+")) {
  ...
  _getRowContentForTable(settings, note) {
    return `|${this._rowImage(settings, note)}|${this._rowTitle(note)}|${this._rowFields(settings, note)}|`;
  },
